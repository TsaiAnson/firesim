...
case class Input(en: Boolean, we: Boolean, keyRe: Int, keyWr: Int, dataWr: Int)
case class Output(found: Boolean, dataRe: Int)
...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    var out = swcam.process(Input(true, false, 123, 0, 0))
    println(out.found)
    out = swcam.process(Input(true, false, 0, 0, 0))
    println(out.found)
    out = swcam.process(Input(true, true, 0, 123, 123))
    println(out.found)
    out = swcam.process(Input(true, true, 0, 456, 789))
    println(out.found)
    out = swcam.process(Input(true, true, 0, 0, 7809))
    println(out.found)
  }
}
